...
import { mochaHooksRest } from '../testData/integrationHook';

use(spies);

describe('RestLayoutService', () => {
  const { apiKey, apiHost, siteName, env, response, responseUrl } = mochaHooksRest();
  ...
  it('should fetch layout data', () => {
    ...
    return service
      ...
      .then((layoutServiceData: LayoutServiceData & AxiosRequestConfig) => {
        ...
        if (env === 'dev') {
          expect(layoutServiceData.url).to.equal(responseUrl);
          expect(layoutServiceData.data).to.deep.equal(response);
          return;
        }
        expect(layoutServiceData).to.deep.equal(response);
      });
  });
