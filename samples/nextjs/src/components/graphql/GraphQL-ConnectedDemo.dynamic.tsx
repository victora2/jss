...
console.log('outside connected demo');

const GraphQLConnectedDemo = (props: any): JSX.Element => {
  console.log('gql connected demo props', props);
  const data = props.rendering.uid
    ? useComponentProps<GraphQLConnectedDemoData>(props.rendering.uid)
    : undefined;
  console.log(props.rendering.uid, data);
  console.log(
    props.rendering.uid && useComponentProps<GraphQLConnectedDemoData>(props.rendering.uid)
  );

  return (
    <div data-e2e-id="graphql-connected">
      {console.log('inside connected demo')}
